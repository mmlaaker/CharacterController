...
{	
	...
	public void SetGroundInfo( CollisionInfo collisionInfo , CharacterActor characterActor , bool setGroundContact = false )
	{
	...
	void SetGroundContact( CollisionInfo collisionInfo , CharacterActor characterActor )
	{
		if( !collisionInfo.collision )
			return;
		
		Contact groundContact = new Contact();
		groundContact.point = collisionInfo.hitInfo.point;
		groundContact.normal = collisionInfo.hitInfo.normal;

		Vector3 pointVelocity = Vector3.zero;		
		if( characterActor.Is2D )
		{
			if( collisionInfo.hitInfo.rigidbody2D != null )
				pointVelocity = collisionInfo.hitInfo.rigidbody2D.GetPointVelocity( groundContact.point );
		}	
		else
		{
			if( collisionInfo.hitInfo.rigidbody3D != null )
				pointVelocity = collisionInfo.hitInfo.rigidbody3D.GetPointVelocity( groundContact.point );
		}	
		groundContact.pointVelocity = pointVelocity;
		groundContact.relativeVelocity = characterActor.Velocity - groundContact.pointVelocity;
