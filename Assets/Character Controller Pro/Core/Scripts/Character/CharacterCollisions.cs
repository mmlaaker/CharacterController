...
{   
    ...
    void UpdateCollisionInfo( out CollisionInfo collisionInfo , Vector3 position , HitInfo hitInfo , Vector3 castDisplacement , float skin , bool calculateEdge = true , HitInfoFilter hitInfoFilter = new HitInfoFilter() )
    {
        ...
        {            
			...
			if( calculateEdge )
			{
				Vector3 edgeCenterReference = characterActor.GetBottomCenter( position + collisionInfo.displacement , 0f );
            	UpdateEdgeInfo( ref collisionInfo , edgeCenterReference , hitInfoFilter );
			}
        }
        else
        {
	...
	void UpdateEdgeInfo( ref CollisionInfo collisionInfo , Vector3 edgeCenterReference , HitInfoFilter hitInfoFilter )
    {

        Vector3 castDirection = ( collisionInfo.hitInfo.point - edgeCenterReference ).normalized;
